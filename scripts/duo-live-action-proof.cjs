...
function prepare(destination,builder,variant='complete') {
 ...
 const run=duo.start(root,'#155 synthetic CLI proof',builder,{goal:'goal.md',task:'task.md',objective:'Prove the actual customer total and its command test',finish:'All indexed criteria and outstanding instructions independently verified',criteria,typesafe:{mode:'off'}},builder==='claude-code'?hostSession:undefined);
 ...
 const meta={root,builder,variant,runId:run.id,hostSession,syntheticProduct:true,realModels:true,headlessDriver:true,installedSkill:builder==='claude-code'?'.claude/skills/duo-build/SKILL.md':'.agents/skills/duo-build/SKILL.md',runtime:run.runtime,invocation:{exe:builder==='claude-code'?'claude':'codex',args}};
...
function run(destination,builder,variant) {
 const prepared=prepare(destination,builder,variant),{meta,prompt,token}=prepared;
