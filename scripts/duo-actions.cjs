...
function apply(state, result, request, source, round) {
  ...
  for (const row of rows) {
    ...
    for (const file of row.evidence) if (!request.batch.evidence.includes(file) || !result.inspected.includes(`tree/${file}`)) throw Error(`Instruction ${s.id} evidence ${file} must be submitted in batch.evidence and listed as tree/${file} in top-level inspected`);
