...
function invocation(builder, dir, github = null) {
  ...
  if (peer(builder) === 'claude') return { exe: 'claude', env, args: ['-p', '--effort', 'medium', '--output-format', 'stream-json', '--verbose', '--json-schema', JSON.stringify(schema), '--permission-mode', 'dontAsk', '--tools', github ? 'Read,Glob,Grep,Bash' : 'Read,Glob,Grep', '--allowedTools', ['Read,Glob,Grep', ...(github ? reads.map(cmd => `Bash(gh ${cmd} *)`) : [])].join(','), '--disable-slash-commands', '--strict-mcp-config', '--mcp-config', '{"mcpServers":{}}', '--no-session-persistence', '--setting-sources', '', '--safe-mode'] };
...
function reviewLocked(root, dir, state, batch, invoke) {
  ...
  try {
    ...
    const fullPrompt = prompt + direction.prompt + actions.prompt;
