...
const responseSchema = { anyOf: ['complete', 'continuing'].map(kind => {
  const branch = structuredClone(schema);
  branch.properties.assessment.enum = kind === 'complete' ? ['complete'] : ['on_track', 'redirect', 'blocked'];
  branch.properties.next_steps[kind === 'complete' ? 'maxItems' : 'minItems'] = kind === 'complete' ? 0 : 1;
  return branch;
}) };
