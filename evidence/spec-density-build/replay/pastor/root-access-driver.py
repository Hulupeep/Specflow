...
try:
 ...
 print(json.dumps({'status':'observed','archive_opened':True,'root_folder_read':True,'private_content_emitted':False,'limitation':'No folder enumeration or message content inspected'}))
 archive.close()
...
with tempfile.TemporaryDirectory(dir=base) as d:
 ...
 try:
  ...
  parsed.update({'sandbox_network':'denied','source_write':'denied','memory_limit_gib':2,'worker_time_limit_seconds':20,'scope':'One representative PST, archive open and root access only; previous structure walk failed, not extraction qualification'})
