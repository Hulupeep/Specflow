...
function fixture(builder='claude-code',extraCriteria=0) {
 ...
 function peer(edit=()=>{}) { return (exe,args,opts)=>{
  ...
  edit(result,req,opts);if(exe==='codex')fs.writeFileSync(path.join(opts.cwd,'response.json'),JSON.stringify(result));return JSON.stringify({structured_output:result});
 };}
