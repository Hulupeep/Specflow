...
After mapping test files, check if ALL tests in the file are skipped skeletons:

```bash
grep -c "test.skip(true" tests/e2e/journey_*.spec.ts
```

If every `test()` block in a file contains `test.skip(true, 'Skeleton`, report:
```
JOURNEY GATE TIER 1: FAIL
Reason: All tests are unimplemented skeletons.
Issue: #<N>
File: tests/e2e/journey_{name}.spec.ts
Tests: 5 total, 5 skipped (all skeleton)
Action: Implement test bodies before running journey gate.
```
EXIT 1 -- Skeleton-only files do not count as test coverage.
...
- [ ] Missing test files BLOCK (EXIT 1), not warn — specflow-writer Step 10b is mandatory
- [ ] Skeleton-only test files BLOCK (EXIT 1) — tests must be implemented
