...
- **Spec-build loop (idea → defensible tickets)**: the upstream loop — discovery → thin backlog → review of selected decisions → applicable uplift and simulation for the next build-ready slice. Runs in gmh-docs (PRDs/journeys live here), drives the [adversarial-prd-reviewer](https://github.com/Hulupeep/adversarial-prd-reviewer) + Specflow pipeline, and requires a human-approved SHIP verdict before GitHub issues are created; local thin backlog capture may precede review. Contract + driver in [`spec-build-loop.md`](./spec-build-loop.md).
