...
3. Advance exactly ONE rail; persist the diff + an evidence note (tests WHERE/WHICH/HOW MANY/SKIPPED, oracle for each asserted number, provenance source → transform → API/input → output → assertion, audit row per state mutation).
